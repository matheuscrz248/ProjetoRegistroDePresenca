...
const TableEditor3 = () => {
  ...
  const carregarDados = async () => {
    try {
      const alunosArmazenados = await AsyncStorage.getItem('alunos_turma3');
      if (alunosArmazenados !== null) {
        setAlunos(JSON.parse(alunosArmazenados));
      } else {
        setAlunos([
          { id: 1, nome: 'Rafael', faltas: [false, false], totalFaltas: 0 },
        ]);
      }
    } catch (error) {
  ...
  const salvarDados = async () => {
    try {
      await AsyncStorage.setItem('alunos_turma3', JSON.stringify(alunos));
