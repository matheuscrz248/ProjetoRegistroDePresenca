...
const TableEditor1 = () => {
  ...
  const carregarDados = async () => {
    try {
      const alunosArmazenados = await AsyncStorage.getItem('alunos_turma1');
      if (alunosArmazenados !== null) {
        setAlunos(JSON.parse(alunosArmazenados));
      } else {
        setAlunos([
          { id: 1, nome: 'João', faltas: [false, false], totalFaltas: 0 },
          { id: 2, nome: 'Maria', faltas: [true, false], totalFaltas: 1 },
          { id: 3, nome: 'Pedro', faltas: [false, true], totalFaltas: 1 },
        ]);
      }
    } catch (error) {
  ...
  const salvarDados = async () => {
    try {
      await AsyncStorage.setItem('alunos_turma1', JSON.stringify(alunos));
