...
const TableEditor5 = () => {
  ...
  const carregarDados = async () => {
    try {
      const alunosArmazenados = await AsyncStorage.getItem('alunos_turma5');
      if (alunosArmazenados !== null) {
        setAlunos(JSON.parse(alunosArmazenados));
      } else {
        setAlunos([
          { id: 1, nome: 'Sabrina', faltas: [false, false], totalFaltas: 0 },
        ]);
      }
    } catch (error) {
  ...
  const salvarDados = async () => {
    try {
      await AsyncStorage.setItem('alunos_turma5', JSON.stringify(alunos));
